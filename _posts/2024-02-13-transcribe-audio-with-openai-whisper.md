...
description: Enable GPU support on an OpenFaaS cluster to transcribe audio with OpenAI Whisper.
date: 2024-02-13
categories:
- serverless
- gpu
...
In this tutorial we will show how NVIDIA GPU support can be enabled on a OpenFaaS K3S cluster and used to run GPU accelerated workloads inside your functions. As an example we will create a function that uses the [OpenAI whisper model](https://github.com/openai/whisper) to transcribe audio to text.
...
There are still some improvements to be made on our side to fully support GPU workloads. Currently the ability to request custom GPU resources for functions through `limits` and `requests` is not available.
